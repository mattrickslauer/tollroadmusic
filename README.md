...
- **AWS Lambda — the projector.** TollRoad runs **full polyglot CQRS**: the command path writes **only** to DynamoDB (the conditional debit + meter/top-up events), and the projector Lambda is the **sole writer of the DSQL read models**. It consumes DynamoDB Streams (**at-least-once**, so duplicates are expected) and builds the append-only `royalty_ledger` (SoR) **idempotently** (UNIQUE idempotency key + `ON CONFLICT DO NOTHING`), the per-artist/day summaries, the top-up history, and the eventually-consistent reconciliation balance — batched within DSQL's 3,000-row / 10 MiB / 5-min transaction limits. **Rollup-ordering fix:** the synchronous charge used to write the DSQL ledger itself, front-running the rollup so the projector always hit `ON CONFLICT DO NOTHING` and the summaries never updated; now that the command touches only DynamoDB and the projector owns DSQL, that conflict is gone by construction.
...
| Roll up to the ledger | Streams → projector Lambda (sole DSQL writer) → idempotent insert into DSQL ledger, dedup on `UNIQUE(user,track,minute)`, chunked ≤3,000 rows/txn |
