...
export function cosineSimilarity(a: number[], b: number[]): number {
  ...
  let normA = 0;
  let normB = 0;
  // noUncheckedIndexedAccess: a[i] is number | undefined. Read each pair once and
  // skip any hole rather than asserting — a length mismatch then yields a real
  // score instead of NaN poisoning the whole ranking.
