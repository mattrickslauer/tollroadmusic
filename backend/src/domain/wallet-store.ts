...
export async function debitMinute(input: DebitInput): Promise<DebitResult> {
  ...
  try {
    ...
    if (reasons) {
      ...
      if (reasons[1]?.Code === "ConditionalCheckFailed") {
        return { ok: true, balanceMillicents: await readBalance(input.accountId), charged: false };
      }
  ...
  return { ok: true, balanceMillicents: await readBalance(input.accountId), charged: true };
}
